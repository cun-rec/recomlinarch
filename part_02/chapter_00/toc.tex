...
%	\begin{description}
%		\item[add] Adds an instance to a specified \pod{}. Use the parameter \emph{--scale\_role} followed by an instance type from . The rest of the parameters are the same as shown in the last section.
%		\item[clone]
%		\item[recovery]
%		\item[snapshot]
%		\item[update]
%		\item[terminate] Terminates a pod. The rest of the arguments are the same as 
%		\item[tag]
%		\item[rollback]
%		\item[reinstantiate]
%		\item[stop]
